...
function lettersOnly(str){ const s=(str||"").replace(/[^A-Za-z]/g,""); return s || "Untitled"; }
function uniq(base, used){ let n=base; if(!used.has(n)){ used.add(n); return n; } const abc="abcdefghijklmnopqrstuvwxyz"; let i=0; while(true){ let k=i, suf=""; do { suf = abc[k%26]+suf; k=Math.floor(k/26)-1; } while(k>=0); const c=base+suf; if(!used.has(c)){ used.add(c); return c; } i++; } }
...
export default function Home(){
  ...
  useEffect(() => { const unsub = subscribeAuth((u)=>setUser(u)); return () => { if(typeof unsub==='function') unsub(); }; }, []);

  // Load saved API key per provider
  useEffect(() => {
    try { const saved = localStorage.getItem(`fns:key:${provider}`) || ""; setApiKey(saved); } catch {}
  }, [provider]);
  ...
  useEffect(() => {
    function onErr(e){ const m=(e?.error && e.error.message) || e?.message || "Client error"; try { console.error(e); } catch{}; try { toast.error(m); } catch{} }
    window.addEventListener("error", onErr);
    return () => window.removeEventListener("error", onErr);
  }, []);

  const handleLogin = async () => { const u = await login(); if(u) toast.success("Logged in!"); };
  ...
  async function validateKey(){
    ...
    if(data.ok){ localStorage.setItem(`fns:key:${provider}`, apiKey); toast.success("API key saved!"); } else { toast.error(data.error || "Invalid key"); }
  }

  function handleImport(e){
    const sel = Array.from(e.target.files||[]).filter(isVector);
    if(!sel.length){ toast.error("No SVG/EPS/AI files found."); return; }
    setFiles(sel);
    setPreviews(sel.map(f=>({ name:f.name, url: URL.createObjectURL(f) })));
    setProgress(0); setRenamedCount(0); setRenamedMap({}); used.current = new Set();
  ...
  async function renameOne(i){
    ...
    try {
      const fd = new FormData(); fd.append("file", file); fd.append("key", apiKey); fd.append("provider", provider);
      const res = await fetch("/api/rename", { method: "POST", body: fd });
      const data = await res.json();
      let s = lettersOnly(data?.newName || "Untitled"); s = uniq(s, used.current);
      setRenamedMap(m=>({ ...m, [i]: s })); setRenamedCount(c=>c+1); return s;
  ...
  async function exportZip(){
    ...
    const { default: JSZip } = await import("jszip"); // dynamic import (safer)
    const zip = new JSZip();
    for(let i=0;i<files.length;i++){
      ...
      <main className="max-w-7xl mx-auto p-4 md:p-6">
        ...
        <header className="flex items-center gap-3 justify-between mb-6">
          ...
          <div className="flex items-center gap-3">
            ...
            {user ? (
              ...
              <button onClick={handleLogin} className="px-3 py-1.5 rounded bg-indigo-600 text-white text-sm">Login with Google</button>
            )}
          </div>
        </header>
        ...
        <section className="bg-white rounded shadow p-4 mb-6">
          <div className="flex flex-wrap items-center gap-3">
            ...
            <button onClick={validateKey} className="px-3 py-1.5 rounded bg-green-600 text-white text-sm">Save Key</button>
        ...
        <footer className="flex flex-col md:flex-row items-center justify-between gap-2 text-sm text-gray-600 mt-8">
          <div className="flex items-center gap-2">
            <a href="https://www.facebook.com/anil.chandrabarman.3" target="_blank" rel="noopener noreferrer" className="underline">Facebook</a>
            <span>•</span>

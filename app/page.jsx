...
export default function Home(){
  ...
  useEffect(()=>{ function onErr(e){ const m=(e?.error&&e.error.message)||e?.message||"Client error"; try{ console.error(e);}catch{}; toast.error(m);} window.addEventListener("error", onErr); return ()=>window.removeEventListener("error", onErr); },[]);

  async function handleLogin(){ if(!isValidEmail(emailInput)) return toast.error("Enter a valid, non-disposable email."); const u = await loginWithEmail(emailInput); if(u){ setUser(u); toast.success("Logged in!"); } else { toast.error("Login failed."); } }
  ...
  function handleImport(e){
    ...
    setFiles(sel); setPreviews(sel.map(f=>({ name:f.name, url: URL.createObjectURL(f) })));
  ...
  async function start(){
    ...
    for(let i=0;i<files.length;i++){
      ...
      try{ await renameOne(i); } catch(e){ console.error(e); toast.error(String(e?.message||e)); }
    ...
    <ErrorBoundary>
      <main className="max-w-7xl mx-auto p-4 md:p-6">
        ...
        <section className="bg-white rounded shadow p-4">
          ...
          {files.length===0 ? <div className="text-sm text-gray-600">No files imported yet.</div> : (
              ...
              {files.map((f, idx)=>(
                <div key={idx} className="border rounded p-3 flex flex-col bg-gray-50">
                  <div className="h-32 flex items-center justify-center bg-white border rounded overflow-hidden mb-2">
                    {isSVG(f) ? <img src={(previews[idx] && previews[idx].url) || ""} alt={f.name} className="max-h-28" /> : <div className="text-xs text-gray-500 text-center p-2">{f.name.toLowerCase().endsWith(".eps")?"EPS Preview not supported":f.name.toLowerCase().endsWith(".ai")?"AI Preview not supported":"Preview unavailable"}</div>}
                  </div>
                  <div className="text-xs break-all text-gray-700 mb-1">Old: {f.name}</div>
                  <div className="text-xs font-semibold break-all text-emerald-700 mb-2">New: {(renamedMap[idx]?renamedMap[idx]:"—")+(renamedMap[idx]?"."+f.name.split(".").pop():"")}</div>
